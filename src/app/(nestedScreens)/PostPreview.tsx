...
export default function PostPreview(){
    ...
    const { media, singleMedia, imgResizeMode } = useLocalSearchParams()
      ...
      <View style={{flex: 1, marginTop: '15%'}}>
        {
          ...
          <View>
              {
                theSingleMedia.mediaType == 'photo'?  <Image source={{ uri: theSingleMedia.uri}} style={{width: width, height: 50/100 * height, borderRadius: 5}} resizeMode={resizeImage? 'contain' : 'cover'}/>
                :   
              ...
              { theSingleMedia.mediaType == 'photo' && <View style={{position:'absolute', top: (50/100 * height) - 50, left: '5%', width: 40, height: 40, borderRadius: 40, justifyContent:'center', alignItems:'center', backgroundColor:'rgba(0,0,0,0.8)'}}>
            ...
            renderItem={({item, index})=>{
              ...
              <Pressable style={{ width: width, height: 50/100 * height, paddingHorizontal:10}} onPress={()=>{ setSelectedItem(item); setShowEditModal(true)}}>
                  ...
                  <Pressable onPress={()=>{setShowModal(true); setCurrentItem(item)}} style={{ position:'absolute', left: (width - 10) - 40,  top: 10, justifyContent:"center", alignItems:"center", width: 30, height: 30, borderRadius: 25, backgroundColor:'rgba(0,0,0,0.5)'}}>
        ...
        <View style={{marginTop:'auto', paddingHorizontal: 15, marginBottom: 10}}>
           <View style={{flexDirection:'row', alignItems:'center', justifyContent:'space-between'}}>
               ...
               { currentViewableMedia?.item.mediaType == 'photo'? <Pressable onPress={()=>{}} style={{width: 60, height: 40, borderRadius: 10, backgroundColor: 'rgba(192,192,192,0.3)', justifyContent:'center', alignItems:'center'}}>
